...
/**
 * Script final para corrigir os últimos erros
 */

const fs = require('fs');
const path = require('path');

console.log('🔧 Aplicando correção dos últimos erros...\n');
...
// Correção 1: route.ts - Corrigir erro de tipo com process.env
function fixRouteEnvErrorFinal() {
  ...
  // Corrigir o erro de tipo com process.env - usar uma abordagem diferente
  const fixedContent = content.replace(
    /`\${\(process\.env\.NEXT_PUBLIC_BASE_URL as string\) \|\| 'http:\/\/localhost:3000'}\/api\/illustrations\/search`/g,
...
// Correção 2: IllustrationSearch.tsx - Encontrar e corrigir o img específico
...
async function main() {
  ...
  const fixes = [
    { name: 'Erro de tipo process.env final', fn: fixRouteEnvErrorFinal },
