  ...
  const sendMessage = useCallback(async (
    ...
    try {
      ...
      if (contentType?.includes('application/json')) {
        ...
        if (jsonData.error) {
          console.error('[AISDKUltraChat] Received JSON error response:', jsonData)
        ...
        console.log('[AISDKUltraChat] Received structured response:', jsonData)
        ...
        if (jsonData.trace?.module) {
          try {
            ...
            console.warn('[AISDKUltraChat] Failed to switch module:', e)
          }
        }
