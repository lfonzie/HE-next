...
export function useFastChat(onStreamingStart?: () => void) {
  ...
  const sendMessage = useCallback(async (
    ...
    if (!message || typeof message !== 'string' || message.trim() === '') {
      console.error('[FastChat] Invalid message:', { message, type: typeof message });
      throw new Error('Invalid message: message must be a non-empty string');
    }

    const trimmedMessage = message.trim();
    if (!trimmedMessage) {
      console.error('[FastChat] Message is empty after trimming:', { originalMessage: message });
      throw new Error('Message cannot be empty');
    }

    console.debug('[FastChat] Sending message:', { 
    ...
    try {
      ...
      // Include conversation history for context (reduzido para 5 mensagens)
      ...
      const requestBody = {
        ...
        module: finalModule,
        conversationId,
        history: conversationHistory.slice(-5) // Reduzido de 10 para 5
      }

      console.debug('[FastChat] Request body:', {
      ...
      if (contentType?.includes('application/json')) {
        ...
        if (jsonData.error) {
          console.error('[FastChat] Received JSON error response:', jsonData)
        ...
        console.log('[FastChat] Received structured response:', jsonData)
        ...
        if (jsonData.trace?.module) {
          try {
            ...
            console.warn('[FastChat] Failed to switch module:', e)
          }
        }
      ...
      if (!reader) {
        console.error('[FastChat] No response reader available')
      ...
      try {
        ...
        console.warn('[FastChat] Falha ao alternar módulo automaticamente:', e)
      }
      ...
      try {
        ...
        console.error('[FastChat] Streaming error:', streamError)
