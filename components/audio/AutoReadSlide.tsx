...
export default function AutoReadSlide({
  text,
  className = '',
  voice = 'Orus',
  onAudioStart,
  ...
  }, [isConnected, isConnecting, voice, initAudioContext, text, onError]);
  ...
  const getStatusIcon = () => {
    ...
    if (isReading) return <Volume2 className="w-4 h-4" />;
  ...
  const getStatusColor = () => {
    ...
    if (isReading) return 'bg-green-100 text-green-800';
        ...
        <div className="space-y-4">
          ...
          {process.env.NODE_ENV === 'development' && (
            <div className="text-xs text-gray-500 space-y-1">
              ...
              <div>Voz: {voice}</div>
              <div>Texto: {text?.substring(0, 50)}...</div>
            </div>
          )}
