...
import { X, Search, Newspaper, Calendar, User, ExternalLink, Loader2, Filter } from 'lucide-react';
import { NewsArticle, NewsAPIService } from '@/lib/services/newsapi-service';
...
export function NewsAPIModal({ isOpen, onClose, searchQuery = '', className = '' }: NewsAPIModalProps) {
  ...
  const searchNews = async (query: string) => {
    ...
    try {
      ...
      if (query.trim()) {
        result = await NewsAPIService.searchNews(query, 'pt', 10);
      } else if (selectedCategory) {
        result = await NewsAPIService.getTopHeadlines('br', selectedCategory, 10);
      } else {
        result = await NewsAPIService.getTopHeadlines('br', undefined, 10);
      }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className={`bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] shadow-2xl ${className}`}>
        {/* Header */}
        <div className="relative bg-gradient-to-r from-red-600 to-orange-600 rounded-t-2xl p-6 text-white">
          <button
            onClick={onClose}
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 className="text-2xl font-bold">Notícias Atuais</h2>
              <p className="text-red-100">Mantenha-se informado com as últimas notícias</p>
            </div>
          </div>
        ...
        <div className="p-6 max-h-[60vh] overflow-y-auto">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {article.description && (
                          <p className="text-sm text-gray-700 mb-3 line-clamp-3">
                            {NewsAPIService.formatDescription(article.description)}
                          </p>
                        )}
                        ...
                        <div className="space-y-1 text-sm text-gray-600">
                          <div className="flex items-center gap-2">
                            ...
                            <span>{NewsAPIService.formatDate(article.publishedAt)}</span>
                          </div>
          ...
          {!searchTerm && !selectedCategory && !isLoading && (
            <div className="text-center py-12">
              <Newspaper className="w-16 h-16 mx-auto text-gray-400 mb-4" />
              <h3 className="text-lg font-semibold text-gray-900 mb-2">Explore as notícias</h3>
              <p className="text-gray-600">Digite palavras-chave para buscar ou selecione uma categoria para ver as principais notícias.</p>
            </div>
          )}
        </div>
      </div>
