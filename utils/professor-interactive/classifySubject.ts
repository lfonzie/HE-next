...
export async function classifySubject(query: string): Promise<string> {
  try {
    ...
    if (process.env.OPENAI_API_KEY) {
      const completion = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: `Classifique a seguinte pergunta educacional em uma das categorias: ${SUBJECT_CATEGORIES.join(', ')}. 
      ...
      if (classification && SUBJECT_CATEGORIES.includes(classification)) {
  ...
  return classifySubjectByKeywords(query);
}

function classifySubjectByKeywords(query: string): string {
  ...
  if (lowerQuery.includes('matemática') || lowerQuery.includes('matematica') || 
      ...
      lowerQuery.includes('número') || lowerQuery.includes('numero')) {
    return 'Matemática';
  }
