 ...
 * @param {number} minTokens - Mínimo de tokens requerido (padrão: 130)
 * @returns {Object} - Resultado da validação
 */
export function validateSlideTokens(slide, minTokens = 130) {
...
function generateRecommendations(slideValidations, duration) {
  ...
  if (shortSlides.length > 0) {
    recommendations.push({
      type: 'warning',
      message: `${shortSlides.length} slide(s) com menos de 130 tokens. Considere expandir o conteúdo.`
    });
  }
