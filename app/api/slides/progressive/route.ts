...
async function generateProgressiveSlide(
  topic: string, 
  position: number, 
...
${previousContext}`;
  ...
  try {
    ...
    if (attempt < 3) {
      ...
      return generateProgressiveSlide(topic, position, previousSlides, attempt + 1);
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    while (attempts < maxAttempts) {
      try {
        slide = await generateProgressiveSlide(topic, position, previousSlides, attempts);
